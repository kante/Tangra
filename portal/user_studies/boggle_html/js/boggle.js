...
var Boggle = Class.create({
    ...
    submitWord: function(word) {
        ...
        // TODO: log data to server here ???
       
        wordAttributes.valid = all_words[word.toLowerCase()] == 1;
        wordAttributes.duplicate = this._wordList.containsWord(word);
        if (wordAttributes.valid && ! wordAttributes.duplicate) {
            wordAttributes.score = this._scores[word.length];
        } else {
            wordAttributes.score = 0;
        }
        this._wordList.addWord(word, wordAttributes);
...
var WordList = Class.create({
    ...
    addWord: function(word, wordAttributes, bottom) {
        ...
        this._words[word.toLowerCase()] = true;
